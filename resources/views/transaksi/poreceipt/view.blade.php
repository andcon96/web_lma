...
<form method="post" action="{{route('submitReceipt')}}" id='submit'>
  ...
  <div class="row mb-3">
    ...
    <div class="col-md-8">
      <input type="text" class="form-control" name="remarkreceipt" maxlength="24" value="{{ old('remarkreceipt') }}" />
    </div>
  </div>
  <div class="row mb-3">
    <label for="nopol" class="col-form-label col-md-3" style="margin-left:25px">{{ __('No. Polisi') }}</label>
    <div class="col-md-8">
      <textarea type="text" class="form-control" name="nopol" rows="3" maxlength="30">{{ old('nopol') }}</textarea>
    </div>
  </div>
